...
private struct CharacterPickerPreviewIcon: View {
    ...
    private static func image(for character: RunCatCharacter, frameIndex: Int) -> NSImage? {
        ...
        if loadedImage == nil, let resourcePath = Bundle.main.resourcePath {
            loadedImage = NSImage(contentsOf: URL(fileURLWithPath: "\(resourcePath)/RunCat/\(character.id)/frame_\(safeFrameIndex).png"))
        }
