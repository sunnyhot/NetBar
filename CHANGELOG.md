...
- 默认网络速度动画不再采集未使用的 CPU、内存与热状态指标；只有对应动画速度来源启用时才启动系统资源采样
- `nettop` 与进程资源读取改为并行执行，并减少应用流量刷新、历史配置和高流量检测造成的重复 SwiftUI 发布
- 网络采样与 RunCat 动画计时器接入 Common RunLoop 并配置容差，滚动详情、操作菜单时保持更新连续，同时允许系统合并唤醒
- 趋势图归一化数据移入展示模型统一计算，减少 SwiftUI 重绘期间的数组创建和重复遍历
- 偏好设置新增 `⌘1`～`⌘4` 页面快捷键；颜色预设支持键盘焦点和中英文 VoiceOver 标签
- 动画与材质遵循 macOS“减少动态效果”和“减少透明度”辅助功能设置，并新增采样策略、历史发布与图表数据回归测试
...
- 将偏好设置中的“智能”改为更准确的“提醒与历史”，把“智能检测”收敛为持续高流量提醒
- 保留原有提醒阈值、通知授权和历史设置的数据结构，升级后不丢失用户配置
- 详情页默认只展示系统主网络接口；系统未标记主接口时，回退到当前活动接口或首个已识别接口
- 将完整接口列表、累计流量和收发包数移入默认收起的“高级接口诊断”，减少详情页视觉负担
- 补充中英文文案、展开控件无障碍提示、主接口选择测试和界面结构回归测试
...
- 移除重复展示本地接口状态的“网络可用”健康卡片，详情弹窗不再用大面积卡片重复说明系统接口可用
- 移除“网络状态正常”卡片及重复的智能设置入口，趋势图或首次引导现在直接承接顶部状态区
- 保留顶部最新异常提示、系统通知，以及菜单栏和智能角色使用的本地接口在线/离线反馈
- 清理仅供已删除卡片使用的原因、指标、建议、文案和测试，减少无效展示层与维护成本

## v0.40.3 (2026-07-27)

### Cleanup — 移除代理归因告警

- 移除基于单次采样和进程名关键词判断的代理/VPN 归因差异告警，避免旧应用样本与当前接口速率组合造成误报
- 移除对应偏好设置、异常事件、系统通知、健康原因映射和智能角色反馈
- 保留应用面板中的接口与应用流量覆盖率，以及代理/VPN、子进程和系统服务标签，继续作为只读归因说明
- 精简相关测试和项目文档，不保留已删除告警字段的兼容分支

## v0.40.2 (2026-07-27)

### Cleanup — 移除主动网络诊断

- 移除 DNS/HTTPS 主动探测、诊断调度、阈值评估和手动重测入口，不再产生额外公网请求
- 移除主动诊断偏好设置、同意说明、GitHub 参考目标及相关诊断输出
- 网络健康状态精简为本地接口“可用 / 离线”两态，保留异常提醒、智能菜单栏和角色反馈
- 删除旧主动诊断字段兼容测试，以及不再可达的“波动 / 较差”状态与展示分支

## v0.40.1 (2026-07-27)

### Cleanup — 精简智能与宠物功能

- 移除宠物状态、技能、提醒及其偏好设置和网络事件联动，减少常驻状态与维护成本
- 移除洞察事件流、洞察建议、聚合器及相关设置项，不再保留或展示异常事件卡片
- 保留网络异常检测、最新网络状态卡、系统通知、网络健康诊断与智能菜单栏反馈
- 旧版洞察设置字段升级后会被安全忽略，重新保存时不再写入

## v0.40.0 (2026-07-10)

### Feature — 网络健康层与智能菜单栏反馈

- 新增可解释的网络健康模型：`good`/`fluctuating`/`poor`/`offline` 四态，不显示伪精确的 0–100 分数
- 新增主动网络诊断（opt-in，默认关闭）：复用已用于更新的 GitHub 参考目标，测量 DNS 耗时、HTTPS 延迟和成功/失败次数；不上传任何流量、应用或浏览数据
- 详情弹窗趋势图下方新增健康摘要面板，首屏即可看到活动、健康状态、证据、主要原因和建议动作
- 主动诊断需明确同意后启用，同意页说明测量内容与隐私边界；可随时在偏好设置关闭
- 智能菜单栏与智能角色现由统一健康快照驱动：good 保留用户布局、fluctuating 琥珀强调、poor 珊瑚强调、offline 关键红并暂停角色动画
- 参考目标失败（本地路径仍可用）不会产生 offline；高流量/突增/代理归因只成通知，不自动降低健康状态
- 诊断调度自适应：弹窗可见 15s、后台 60s、降级验证 10s（最多 2 分钟）、低电量与锁屏暂停、唤醒即补测
- 新增 42 个网络健康单元测试（阈值边界、滞回、证据模式、调度、智能映射、设置向后兼容）

## v0.39.17 (2026-07-03)

### Enhancement — UI 视觉重构

- 将详情弹窗与偏好设置收敛为更克制的 macOS 原生网络仪表风格，减少大面积薄荷渐变和重卡片层级
- 新增共享信号色、上传、告警、危险和中性色 token，统一弹窗图表、状态 chip、应用流量、接口与系统资源面板
- 偏好设置改为更轻量的设置中心标题、分组选项和选中态，并新增源码边界回归测试锁定视觉体系

## v0.39.16 (2026-07-01)

### Fix — 菜单栏背景间距再收紧

- 将菜单栏背景胶囊左右内边距从 5pt 收到 3pt，减少开启背景时的横向占用
- 保持无背景状态、角色与速率文字间距、字体和行距不变，避免影响现有布局偏好
- 更新状态栏自动宽度回归测试，锁定背景态 3pt 紧凑内边距计算

## v0.39.15 (2026-06-29)

### Fix — 设置页面切换流畅度

- 为菜单栏实时预览新增签名缓存，避免偏好页小开关变化时重复执行状态栏 bitmap 渲染
- 移除内置角色选择网格的常驻 8fps 动画刷新，降低设置页打开后的主线程压力
- 状态栏角色动画配置改为按角色相关设置变化才重建，文字颜色、加粗、箭头等显示开关只触发必要重绘
- 新增设置页性能回归测试，锁定预览缓存、角色网格静态化和动画配置过滤行为

## v0.39.14 (2026-06-29)

### Enhancement — Living Signal UI 与弹窗流畅度

- 重新设计菜单弹窗与偏好设置视觉体系，加入 Living Signal 面板、实时信号图表、洞察卡片和更清晰的控制区层级
- 新增平衡模式状态栏信号策略，让菜单栏在异常、代理归因差异、上传高峰和总速率之间更稳定地切换重点信息
- 优化偏好设置 Tab 切换、菜单弹窗滚动和应用列表下滑性能，移除滚动热路径中的常驻动画、重材质和逐行异步图标加载
- 新增 UI 结构与滚动性能回归测试，锁定轻量 Tab、静态弹窗徽章和无常驻弹窗动画约束

## v0.39.13 (2026-06-24)

### Fix — 菜单栏背景宽度收紧

- 将菜单栏背景态左右内边距从 8pt 收到 5pt，减少开启胶囊背景时的横向占用
- 保持角色与速率文字之间的间距不变，避免影响当前动画角色布局
- 新增状态栏自动宽度回归测试，锁定背景态紧凑内边距计算

## v0.39.12 (2026-06-24)

### Fix — 轮换角色随机炫彩生效

- 修复轮换到“驯鹿与雪橇”这类黑色剪影角色时，随机炫彩仍显示为黑色的问题
- 将剪影角色切回模板染色路径，让动态颜色模式按渐变遮罩正常生效
- 新增菜单栏渲染回归测试，覆盖角色轮换与随机炫彩组合下的彩色像素输出

## v0.39.11 (2026-06-24)

### Enhancement — 角色轮换炫彩入口

- 在“动画与轮换”设置页新增角色颜色模式入口，开启角色轮换时也能直接切换彩虹、随机炫彩等动态颜色
- 将角色页与轮换页的颜色模式选择器改为共享组件，避免两个入口后续行为不一致
- 新增偏好入口回归测试，确保轮换页持续暴露共享的角色颜色模式设置

## v0.39.10 (2026-06-23)

### Fix — 菜单栏弹框与应用列表流畅度

- 应用列表图标解析改为后台异步执行，避免多行应用 badge 同时查询图标时阻塞主线程
- 详情弹框活动监控移除高频鼠标移动事件，并对自动关闭计时器刷新做节流，降低滑动和悬停时的主线程压力
- 新增图标异步解析和弹框活动刷新策略回归测试，防止后续改动重新引入卡顿

## v0.39.9 (2026-06-22)

### Enhancement — 菜单栏详情下滑流畅度与智能角色

- 详情窗口打开时延后接口刷新和应用级采样，避开首帧下滑动画，降低点击菜单栏后的卡顿感
- 详情页滚动内容改为懒构建，应用图标改为延后解析并复用缓存，保持视觉不变同时减少首屏主线程压力
- 智能菜单栏优化异常/应用样本的新鲜度判断、高流量阈值、自动宽度和总速率可见性
- 新增智能角色推荐开关，并扩展柴犬、兔子、企鹅、咖啡杯、小云朵、小盆栽 6 套可爱宠物/小物件角色
- 更新发布链路文档，补充本地打包、GitHub Release workflow 和 LaunchServices 调试坑点
